...
config.gpu_options.per_process_gpu_memory_fraction = 0.1  # 程序最多只能占用指定gpu50%的显存,解决占用问题尝试1
...
class Trainer:
    def __init__(self, options):
        ...
        # Local path to trained weights file
        COCO_MODEL_PATH = "/share/home/linandi/jzw/monodepth2/mask_rcnn_coco.h5"
        if not os.path.exists(COCO_MODEL_PATH):
            utils.download_trained_weights(COCO_MODEL_PATH)
        print('1')
        config = InferenceConfig()
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            target_mask = []
            im_target = tensor_to_np(target)
            mid_target_output = self.predictor.detect([im_target], verbose=0)
            target_mask = get_tf_maskrcnn_mask(mid_target_output, im_target)
            # print(target.shape) [1, 3, 192, 640]
            # todo add detectron2_mask
            # detectron2_mask = []
            pred_mask = [[], []]
            for frame_id in self.opt.frame_ids[1:]:
                pred = outputs[("color", frame_id, scale)]
                im_pred = tensor_to_np(pred)
                mid_pred_output = self.predictor.detect([im_pred], verbose=0)
                pred_mask[frame_id] = get_tf_maskrcnn_mask(mid_pred_output, im_pred)
                # print(frame_id)
                # cv2.imshow("mask", tf_maskrcnn_mask[frame_id])
                # cv2.waitKey(3000)

                # im = tensor_to_np(pred)
                # outputs = self.predictor(im)
                # detectron2_mask[frame_id] = get_detectron2_mask(outputs, im)
                # cv2.imshow("mask", detectron2_mask[frame_id])
                # cv2.waitKey(0)
                ...
                # cv2.imshow('target_mask', target_mask * 255)
                # cv2.imshow('pred_mask', pred_mask[frame_id] * 255)
                # cv2.imshow('mask', target_mask * pred_mask[frame_id] * 255)
                # cv2.waitKey(5000)
